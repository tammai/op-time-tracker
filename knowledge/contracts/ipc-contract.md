...
The bridge was read-only until the single-screen redesign. It now carries exactly one write method, `createTimeEntry`, plus `listTimeEntryActivities` to feed the form's required Activity select (OpenProject rejects an entry with no activity).

Invariants specific to the write path:
- **Numeric ids only across IPC** — never hrefs or paths. The main process Zod-validates the input (`CreateTimeEntryInputSchema`) and builds every `_links` href itself, so no renderer string reaches a request URL.
...
- Errors add two codes: `OPENPROJECT_INVALID_INPUT` (our own validation, pre-request) and `OPENPROJECT_VALIDATION_FAILED` (the 422).
