 ...
 * `defineAppConfig`. In the Vue-only (Vite) install used by this Electron
 * renderer there is no Nuxt runtime, so the live config is injected through
 * the `ui({ ui: { colors } })` option in `electron.vite.config.ts`.
 *
 * This file is kept as the design-handoff source of truth: the `ui.colors`
 * block in `electron.vite.config.ts` mirrors the values here. When the
