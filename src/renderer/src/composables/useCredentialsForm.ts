 ...
 *
 * Security (`.opencode/rules/security.md`): the API key is user-entered and
 * lives only in this composable's reactive state until it is handed to the
 * main process via the typed `testConnection` / `saveCredentials` bridge.
...
export function useCredentialsForm(options: UseCredentialsFormOptions = {}) {
   ...
   * which values passed. The key is used once in the main process for this
  ...
  async function runProbe(): Promise<boolean> {
    ...
    // round-trip. (The main process re-validates too.)
    ...
    // A blank field with a key already stored is valid — the main process
  ...
  async function onSave(event: { data: CredentialsFormState }): Promise<void> {
    ...
    try {
      ...
      await window.openproject.saveCredentials({
        ...
        // Blank → keep the stored key (resolved in the main process).
        ...(apiKey ? { apiKey } : {})
      })
      options.onSaved?.()
    } catch (e) {
