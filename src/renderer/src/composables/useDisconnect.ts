 ...
 * `.opencode/rules/conventions-frontend.md`: components hold no business
 * logic, and query-cache work goes through `useQueryCache()` in the
 * composable/query layer, never in a component.
 *
 * Security (`.opencode/rules/security.md`): the key itself never reaches the
 * renderer, so there is nothing secret to scrub here — `clearCredentials()`
 * removes it from the keychain in the main process. What this composable does
