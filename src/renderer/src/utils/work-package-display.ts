 ...
 * panel.
 *
 * Pure — no Vue, no Electron, no fetch — so the display rules are unit-tested
 ...
 * same reason: `.opencode/rules/conventions-frontend.md` keeps logic out of
 ...
 * top-level `type`/`status` string — see `src/main/schemas/work-packages.ts`.
