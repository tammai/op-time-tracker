/**
 * Pure time/duration helpers shared across the main process and renderer.
 *
 * Lives under `src/shared/` so both the main-process Zod schemas
 * (`src/main/schemas/`) and the renderer's pure aggregation helpers
 * (`src/renderer/utils/`) import it via the same module — never duplicating
 * duration parsing logic across the IPC boundary.
 *
 * This module is **pure**: no Vue, no Electron, no fetch. Unit-testable in
